...

{
  ...
  programs.notmuch.hooks.postNew = let
    ...
    custom = {
      shakti = {
        q = "to:shaktidb@googlegroups.com";
        inbox = false;
      };
      ccv = {
        q = "to:CCV@LISTSERV.BROWN.EDU";
        inbox = false;
      };
      wyzant = {
        q = "from:no-reply@wyzant.com";
        inbox = false;
      };
      uu = {
        q = "from:*@uu.nl";
    ...
    nmtag = newtags: removetags: query:
      ''
        notmuch tag ${lib.concatMapStringsSep " " (t: "+${t}") newtags} ${
          lib.concatMapStringsSep " " (t: "-${t}") removetags
        } "(${query})" '';
    tagTo = tag: to: nmtag [ tag ] [ ] "to:${to}";
    tagAny = newtags: removetags: queries:
      nmtag newtags removetags (lib.concatStringsSep " or " queries);
    tagAnyFrom = newtags: removetags: addresses:
      tagAny newtags removetags (map (a: "from:${a}") addresses);
    tc = tag: q: inbox:
      nmtag [ tag ] ([ "unread" ] ++ (if inbox then [ ] else [ "inbox" ])) q;
  in ''
    ...
    notmuch tag +list ${
      ...
      (builtins.attrValues (lib.mapAttrs (tag: a: "tag:${tag}") addresses))
    }
